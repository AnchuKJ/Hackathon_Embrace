...
import { UserData,BeneficiaryData } from '../interfaces';
...
export class ExampleDatabase1 {
  /** Stream that emits whenever the data has been modified. */
  dataChange: BehaviorSubject<BeneficiaryData[]> = new BehaviorSubject<BeneficiaryData[]>([]);
  get data(): BeneficiaryData[] 
  {
    this.dataChange.value.forEach(child=>{      
        const copiedData = this.data.slice();
        copiedData.push(child);
        this.dataChange.next(copiedData);
      });
...
export class ExampleDataSource1 extends DataSource<any> {
  ...
  /** Connect function called by the table to retrieve one stream containing the data to render. */
  connect(): Observable<BeneficiaryData[]> {
    // Listen for any changes in the base data, sorting, filtering, or pagination
    const displayDataChanges = [
      this._exampleDatabase.dataChange,
      this._sort.sortChange,
    ...
    return observableMerge(...displayDataChanges).pipe(map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.data.slice().filter((item: BeneficiaryData) => {
  ...
  /** Returns a sorted copy of the database data. */
  sortData(data: BeneficiaryData[]): BeneficiaryData[] {
    if (!this._sort.active || this._sort.direction == '') { return data; }
